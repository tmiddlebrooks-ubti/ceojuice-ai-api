...
def summarize_comments(comments: List[dict], api_key: str):
    ...
    if openai and api_key:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[{"role": "user", "content": f"Summarize these comments:\n{text}"}]
        )
...
def detect_anomalies(data: List[dict]):
    ...
    anomalies = [d for d in data if d.get("meterValue", 0) > 2*avg]
    return anomalies
...
def summarize_service_calls(calls: List[dict], api_key: str):
    ...
    if openai and api_key:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[{"role": "user", "content": f"Summarize these service calls:\n{text}"}]
        )
        return response.choices[0].message.content
    return " ".join(text.split()[:100]) + "..."
