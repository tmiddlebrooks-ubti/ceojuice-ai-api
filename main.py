...
from ai_engine import summarize_comments, detect_anomalies, forecast_usage, summarize_service_calls
...
client = CEOJuiceClient(customer_id=CUSTOMER_ID,
                        api_keys={
                            "125": API_KEY_ID125,
                            "29": API_KEY_ID29,
                            "968": API_KEY_ID968,
...
def get_survey_summaries():
    try:
        ...
        summary = summarize_comments(comments, OPENAI_API_KEY)
...
def get_meter_data():
    try:
        ...
        return {"data": data, "anomalies": anomalies, "forecast": forecast}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def post_meter_readings(readings: list):
    try:
        result = client.post_meter_readings(process_id=968, readings=readings)
...
def get_service_call_summaries():
    try:
        ...
        summary = summarize_service_calls(calls, OPENAI_API_KEY)
        return {"summary": summary}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
